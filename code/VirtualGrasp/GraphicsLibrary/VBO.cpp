...
VBO::VBO(Vector<Vertex> * v, Vector<Normal> * n, Vector<Color4> * c, StdList<Face> * f)
{
	vertex_vbo_id = 0;
	normal_vbo_id = 0;
	color_vbo_id = 0;
...
void VBO::update()
{
	if(this->isVBOEnabled)
	{
		...
		{
			update_vbo(&vertex_vbo_id, vertices->size() * sizeof(Vertex), &vertices->front());
...
void VBO::render_wireframe(bool dynamic)
{
	if(dynamic || isDirty)
	{
		update_vbo(&vertex_vbo_id, vertices->size() * sizeof(Vertex), &(*vertices)[0]);
		isDirty = false;
	}
