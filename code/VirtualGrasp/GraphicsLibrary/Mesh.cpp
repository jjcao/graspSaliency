...
void Mesh::mergeWith(const Mesh& other)
{
	...
	for(Vector<Vertex>::const_iterator it = other.vertex.begin(); it != other.vertex.end(); it++)
	{
...
void Mesh::rotateVertices(const Vector<int> & vertices, const qglviewer::Quaternion & q, const Vec & pivot)
{
	foreach(const int& i, vertices)
	{
		vertex[i] = Vertex::RotateAround(vertex[i], pivot, q);
	}
...
void Mesh::draw()
{
	...
	// ===================
	// Debug: 
	for(StdMap<Vertex*, Color4>::iterator it = markers.begin(); it != markers.end(); it++)
	{
		Color4 * c = &it->second;
...
Vector<Vertex> Mesh::getCopyPoints()
{
	return vertex;
}
...
Vector<int> Mesh::getFacesConnected( const Vector<int> & facesIndex, int firstFace)
{
	...
	foreach(Face * f, connectedFaces)
	{
		...
		//testPoints3.push_back(f->center());
	}
